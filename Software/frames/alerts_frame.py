...
class LogFrame(ctk.CTkScrollableFrame):
    ...
    def __init__(self, master):
        ...
        for i in range(len(datalog)):
            ...
            self.label_description = ctk.CTkLabel(self.frame_line, text=datalog[i][2], corner_radius=0)
            self.label_description.grid(row=0, column=2, padx=5, pady=0, sticky="ew")

            self.label_priority = ctk.CTkLabel(self.frame_line, text=datalog[i][3], corner_radius=0)
            self.label_priority.grid(row=0, column=3, padx=5, pady=0, sticky="ew")
            if datalog[i][3] == "Alta":
