...
pH Unknown_Device = pH(20, "found device");   //object which holds the device we'll find

pH *device_list[] = {               //an array of boards used for sending commands to all or specific boards
  &Unknown_Device
};

pH* default_board = device_list[0]; //used to store the board were talking to
...
void setup() {
  ...
  Wire.begin(12, 14);                                       //start the I2C
...
void loop() {
  ...
  if (polling == true) {                  //if polling is turned on, run the sequencer
    //Serial.println("Entre en polling == true");
    Seq.run();
  }
}
