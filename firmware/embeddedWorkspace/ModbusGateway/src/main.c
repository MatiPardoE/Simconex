...
int main(void) {
    ...
    for (;;) {
    		...
    		switch( esp.request.fc ){
    			...
    			case WRITE_MULTIPLE_HOLDING_REGISTERS:
    				modbusWriteMultipleHoldingRegisters(
							&sensorRdo,
							esp.request.slaveAddress,			//slaveAddress
							esp.request.FC16.startingAddress,	//startAddress
							esp.request.FC16.qtyRegs,			//qtyRegs
							esp.request.FC16.byteCount,			//byte count
							esp.request.FC16.regsValue			//DATA
					);
    				break;
    	...
    	if( modbusGetResponse(&sensorRdo) == SUCCES_VAL ){
			...
			switch( sensorRdo.response.fc ){
				...
				case WRITE_MULTIPLE_HOLDING_REGISTERS:
					...
					if(sensorRdo.response.FC16.startingAddress == 123){
						i++;
					}

					modbusResponseWriteMultipleHoldingRegister(
							&esp,
							sensorRdo.response.slaveAddress,
							sensorRdo.response.FC16.startingAddress,
							sensorRdo.response.FC16.qtyRegs
					);
    	...
    	if( (msec_t)(ticks-lastValidRequestFromEsp) > TIMEOUT ){
    		DISABLE_INTERRUPTS();
    		SystemSoftwareReset();
    	}
